    ...
    <div className="flex h-screen w-screen flex-col overflow-hidden max-sm:-mb-16 max-sm:pb-0.5">
      ...
      <div className="navbar flex w-full flex-row content-stretch items-center bg-base-300">
        ...
        >
          {/* hamburger icon */}
          <svg
            className={`swap-off fill-current transition-all duration-700 ${isSidebarOpen ? 'hidden' : ''}`}
          ...
          {/* close icon */}
          <svg
            className={`swap-on fill-current transition-all duration-700 ${isSidebarOpen ? '' : 'hidden'}`}
            xmlns="http://www.w3.org/2000/svg"
            width="32"
            height="32"
            viewBox="0 0 512 512"
        ...
        <div className={`mx-2 flex-1 px-2 `}>Kalamandir</div>
