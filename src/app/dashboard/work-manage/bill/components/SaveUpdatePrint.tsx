...
const PaymentStatus: React.FC<{ bill: IBill }> = ({ bill }) => (
  <div className="flex flex-row items-center justify-between gap-1 max-sm:flex-col">
    <div className="badge badge-primary flex flex-row items-center justify-between gap-1">
      <b>Total:</b>
      <p>{bill.totalAmount}</p>
    </div>
    {bill.discount > 0 && (
      <div className="badge badge-secondary flex flex-row items-center justify-between gap-1">
        <b>Discount:</b>
        <p>{bill.discount}</p>
      </div>
    )}
    <div className="badge badge-accent flex flex-row items-center justify-between gap-1">
      <b>Grand:</b>
      <p>{bill.grandTotal}</p>
    </div>
    <div className="badge badge-success flex flex-row items-center justify-between gap-1">
      <b>Paid:</b>
      <p>{bill.paidAmount}</p>
    </div>
    {bill.dueAmount > 0 && (
      <div className="badge badge-warning flex flex-row items-center justify-between gap-1">
        <b>Due:</b>
        <p>{bill.dueAmount}</p>
      </div>
    )}
    <div
      className={`badge ${bill.paymentStatus === 'Paid' ? 'badge-success' : bill.paymentStatus === 'Partially Paid' ? 'badge-warning' : 'badge-error'} flex flex-row items-center justify-between gap-1`}
    >
      <b>Status:</b>
